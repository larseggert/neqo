...
use super::{
    super::ACK_RATIO_SCALE, ack_bytes, client_default_params, connect_rtt_idle, default_client,
    default_server, fill_cwnd, increase_cwnd, induce_persistent_congestion, new_client, new_server,
    send_something, server_default_params, DEFAULT_RTT,
};
...
fn ack_rate_persistent_congestion() {
    ...
    let mut client = new_client(client_default_params().ack_ratio(ACK_RATIO_SCALE));
    let mut server = default_server();
...
fn ack_rate_server_half_rtt() {
    ...
    let mut client = default_client();
    let mut server = new_server(server_default_params().ack_ratio(ACK_RATIO_SCALE * 2));
    let mut now = connect_rtt_idle(&mut client, &mut server, RTT);
